...
@Component({
    selector: 'jhi-content-green-lights',
    templateUrl: './content-green-lights.component.html'
})
export class ContentGreenLightsComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/content-green-lights'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.currentSearch = '';
        this.router.navigate(['/content-green-lights', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: ContentGreenLights) {
        return item.id;
    }
